...
include!("lib.rs");
pub fn save(c: config::Config) -> std::io::Result<()> {
    if let Ok(contents) = toml::to_string(&c) {
        let p = config::path();
        std::fs::create_dir_all(p.parent().expect("Path was root?"))?;
        std::fs::write(p, contents)
    } else {
        Err(std::io::Error::other("Failed to serialize"))
    }
}
fn main() {
    let opts = eframe::NativeOptions::default();
    let mut config = config::load();
    let mut state = LightingConfig::new();
    eframe::run_simple_native("USC Lights bindings", opts, move |ctx, _frame| {
        egui::CentralPanel::default().show(ctx, |ui| {
            egui::ScrollArea::vertical().show(ui, |ui| {
                ui.horizontal_wrapped(state.render(&mut config));
                ui.end_row();
        ...
        ctx.request_repaint_after_secs(0.05); // Need to update blinking
    })
    .unwrap();
}
    ...
    fn device_selection(&mut self, ui: &mut Ui) {
        for dev in self.devices.iter() {
            ...
            if ui.button(n).clicked() {
                let device = if let Some((dev, _p)) = self
                    .highlighted_device
                    .take()
    ...
    fn output_binding(&mut self, config: &mut HashMap<String, Vec<MappedTarget>>, ui: &mut Ui) {
        ...
        for rep in reports {
            ...
            for out in rep.outputs.iter_mut() {
                ...
                egui::ComboBox::from_label(out.name.as_deref().unwrap_or("Unk"))
                    .selected_text(
                        entry
                            .map(|x| format!("{x}"))
